...
- **MCP task semantics drive the design.** Tools declare `execution(task_support = "required" | "optional")` via `rmcp::tool`; clients invoke via MCP's native `tasks/create` / `tasks/list` / `tasks/info` / `tasks/result` / `tasks/cancel` requests. Do not invent a parallel `__taquba_get_result` polling tool, sync/async builder split, or progress mechanism.
